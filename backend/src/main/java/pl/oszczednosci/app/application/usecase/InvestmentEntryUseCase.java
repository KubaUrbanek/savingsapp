...
public final class InvestmentEntryUseCase implements CreateInvestmentEntryUseCase, UpdateInvestmentEntryUseCase,
 ...
 public InvestmentEntryUseCase(InvestmentEntryRepository repository, InvestmentBackupPort storage, InvestmentTypePolicyRegistry policies, Clock clock, IdGenerator ids) { this.repository=repository; this.storage=storage; this.policies=policies; this.clock=clock; this.ids=ids; }
 ...
 public InvestmentEntry update(UUID id, CreateInvestmentEntryCommand c) { InvestmentEntry e=repository.find(new InvestmentEntryId(id)).orElseThrow(()->new InvestmentEntryNotFoundException(id)); return repository.save(e.update(AssetCategory.of(c.type(),c.subcategory()), PortfolioOwner.of(c.owner()), Money.positive(policies.forType(c.type()).normalizePln(c.valuePln())),c.date(),clock.now())); }
 ...
 public void delete(UUID id){repository.delete(new InvestmentEntryId(id));} public byte[] exportDatabase(){return storage.exportBackup();}
