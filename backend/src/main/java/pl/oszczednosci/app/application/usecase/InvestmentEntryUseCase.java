...
public final class InvestmentEntryUseCase implements CreateInvestmentEntryUseCase, UpdateInvestmentEntryUseCase,
 ...
 public InvestmentEntry create(CreateInvestmentEntryCommand c) { InvestmentEntry e=InvestmentEntry.create(new InvestmentEntryId(ids.nextId()), AssetCategory.of(c.type(),c.subcategory()), c.owner(), Money.positive(c.valuePln()), c.date(), clock.now()); return repository.save(e); }
 public InvestmentEntry update(UUID id, CreateInvestmentEntryCommand c) { InvestmentEntry e=repository.find(new InvestmentEntryId(id)).orElseThrow(()->new InvestmentEntryNotFoundException(id)); return repository.save(e.update(AssetCategory.of(c.type(),c.subcategory()), c.owner(), Money.positive(c.valuePln()),c.date(),clock.now())); }
 public List<InvestmentEntry> list(InvestmentFilter f) { if(f.type()!=null&&f.subcategory()!=null) AssetCategory.of(f.type(),f.subcategory()); return repository.matching(new InvestmentEntryCriteria(f.owner(), Optional.ofNullable(f.type()), Optional.ofNullable(f.subcategory()))); }
