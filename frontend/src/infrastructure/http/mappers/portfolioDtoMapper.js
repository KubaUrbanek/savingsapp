import { InvestmentEntry, InvestmentOperation, InvestmentSubcategory, InvestmentType, OperationType, OwnerId } from '../../../domain/portfolio/index.js';
...
function record(dto, path) {
  if (dto === null || typeof dto !== 'object' || Array.isArray(dto)) throw new MappingError(path, dto, new TypeError('Expected an object'));
  return dto;
}
...
export function mapInvestmentSubcategoryDto(value, path = 'subcategory') {
  ...
  if (value === undefined || value === '') throw new MappingError(path, value, new TypeError('Expected null or a known subcategory'));
...
export function mapOperationTypeDto(value, path = 'operationType') {
  if (!SERVER_OPERATION_TYPES.includes(value)) throw new MappingError(path, value, new TypeError('Unknown server operation type'));
...
export function mapInvestmentDto(value) {
  ...
  return mapped('investment', dto, () => InvestmentEntry.create({ ...dto,
    type: mapInvestmentTypeDto(dto.type), owner: mapOwnerIdDto(dto.owner),
...
export function mapOperationDto(value) {
  ...
  return mapped('operation', dto, () => InvestmentOperation.create({ ...dto,
    operationType: mapOperationTypeDto(dto.operationType), type: mapInvestmentTypeDto(dto.type),
    owner: mapOwnerIdDto(dto.owner), subcategory: mapInvestmentSubcategoryDto(dto.subcategory)
  }));
}
