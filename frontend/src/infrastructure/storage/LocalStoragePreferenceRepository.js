...
export class PreferenceStorageError extends Error {
  constructor(message, cause) { super(message, { cause }); this.name = 'PreferenceStorageError'; }
}
export class PreferenceStorageUnavailableError extends PreferenceStorageError {
  constructor(cause) { super('Preference storage is unavailable', cause); this.name = 'PreferenceStorageUnavailableError'; }
}
export class PreferenceStorageQuotaError extends PreferenceStorageError {
  constructor(cause) { super('Preference storage quota was exceeded', cause); this.name = 'PreferenceStorageQuotaError'; }
}
export class InvalidStoredPreferenceError extends PreferenceStorageError {
  constructor(message, cause) { super(message, cause); this.name = 'InvalidStoredPreferenceError'; }
}
...
export class LocalStoragePreferenceRepository {
  constructor(storage) { this.storageProvider = typeof storage === 'function' ? storage : () => storage; }

  getSelectedOwner() { return this.#read().selectedOwner; }
  setSelectedOwner(selectedOwner) { this.#update({ selectedOwner: OwnerId.parse(selectedOwner) }); }
  getHouseholdGoal() { return this.#read().householdGoal; }
  setHouseholdGoal(householdGoal) { this.#update({ householdGoal: Money.positive(householdGoal, 'householdGoal') }); }
  getStockAllocation() { return this.#read().stockAllocation; }
  setStockAllocation(stockAllocation) { this.#update({ stockAllocation: allocation(stockAllocation, STOCK_SUBCATEGORIES, 'stockAllocation') }); }
  getGlobalAllocation() { return this.#read().globalAllocation; }
  setGlobalAllocation(globalAllocation) { this.#update({ globalAllocation: allocation(globalAllocation, GLOBAL_ASSET_CLASSES, 'globalAllocation') }); }
  ...
  #read() {
    try {
      ...
      const migrated = document?.version === 1
        ? { ...defaults(), ...document.preferences, selectedOwner: document.preferences?.selectedUser ?? document.preferences?.selectedOwner }
        : document?.preferences;
      if (![1, PREFERENCE_SCHEMA_VERSION].includes(document?.version)) throw new TypeError('Unsupported preference schema version');
      ...
      if (error instanceof SyntaxError || error instanceof TypeError) throw new InvalidStoredPreferenceError('Stored preferences are invalid', error);
  ...
  #update(change) { this.#write({ ...this.#read(), ...change }); }
  #write(preferences) {
    try {
      this.#storage().setItem(PREFERENCE_STORAGE_KEY, JSON.stringify({ version: PREFERENCE_SCHEMA_VERSION, preferences }));
