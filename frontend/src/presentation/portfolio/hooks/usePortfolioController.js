...
export function usePortfolioController(useCases, scope, filters) {
  ...
  const commands = React.useMemo(() => ({
    recordPortfolioChange: (command) => runCommand('recordPortfolioChange', () => useCases.recordPortfolioChange.execute(command)),
    deleteInvestmentEntry: (id) => runCommand('deleteInvestmentEntry', () => useCases.deleteInvestmentEntry.execute(id)),
    deleteInvestmentOperation: (id) => runCommand('deleteInvestmentOperation', () => useCases.deleteInvestmentOperation.execute(id)),
    importDatabaseBackup: (file) => runCommand('importDatabaseBackup', () => useCases.importDatabaseBackup.execute(file)),
