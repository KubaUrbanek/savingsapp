...
export const deposit = (asset, amountPln, previousValue) => ({
  kind: PortfolioChangeKind.DEPOSIT,
  asset,
  amountPln,
  previousValue
});
export const withdrawal = (asset, amountPln, previousValue) => ({
  kind: PortfolioChangeKind.WITHDRAWAL,
  asset,
  amountPln,
  previousValue
});
export const valuation = (asset, valuePln) => ({ kind: PortfolioChangeKind.VALUATION, asset, valuePln });
export const buy = (asset, amountPln, previousValue) => ({
  kind: PortfolioChangeKind.BUY,
  asset,
  amountPln,
  previousValue
});
export const sell = (asset, amountPln, previousValue) => ({
  kind: PortfolioChangeKind.SELL,
  asset,
  amountPln,
  previousValue
});
...
export class RecordPortfolioChange {
  constructor(portfolioCommandGateway) {
    this.portfolioCommandGateway = portfolioCommandGateway;
  }
