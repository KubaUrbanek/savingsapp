...
export const OwnerId = Object.freeze({
  parse(value) {
    return requiredString(value, 'owner');
  },
  of(value) {
...
function decimal(value, field) {
  ...
  if (typeof value !== 'string' && typeof value !== 'number')
    throw new DomainValidationError(field, `${field} must be a number`);
...
export const Money = Object.freeze({
  ...
  zeroOrPositive(value, field = 'money') {
    return this.parse(value, { field });
  },
  positive(value, field = 'money') {
    return this.parse(value, { allowZero: false, field });
  }
});
