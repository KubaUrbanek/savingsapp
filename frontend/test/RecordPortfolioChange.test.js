...
test('supports every explicit command variant and delegates one semantic command', async () => {
  ...
  const useCase = new RecordPortfolioChange({
    recordPortfolioChange: async (change) => { calls.push(change); return change; }
  });
  ...
  assert.deepEqual(calls.map(({ nextValue }) => nextValue), [110, 90, 125, 110, 90]);
  assert.deepEqual(calls.map(({ command }) => command.kind), ['DEPOSIT', 'WITHDRAWAL', 'VALUATION', 'BUY', 'SELL']);
});
...
test('returns a field-addressable failure instead of clamping an overdrawn withdrawal', async () => {
  ...
  await assert.rejects(
    ...
    (error) => error instanceof PortfolioChangeValidationFailure
