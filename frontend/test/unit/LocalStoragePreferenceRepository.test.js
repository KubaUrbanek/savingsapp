...
import {
  ...
  FALLBACK_USERS
} from '../../src/domain/portfolio/constants.js';
...
test('migrates legacy keys into the current versioned document', () => {
  const storage = new MemoryStorage({
    ...
    'oszczednosci.stockTargetAllocations': JSON.stringify({
      ZLOTO: 20,
      RYNKI_ROZWINIETE: 60,
      RYNKI_ROZWIJAJACE_SIE: 20
    }),
...
test('validates deserialized domain values', () => {
  const storage = new MemoryStorage({
    [PREFERENCE_STORAGE_KEY]: JSON.stringify({
...
test('maps unavailable and quota-limited storage to typed errors', () => {
  ...
  assert.throws(
    () => new LocalStoragePreferenceRepository(unavailable).getSelectedOwner(),
    PreferenceStorageUnavailableError
  );
  assert.throws(
    () =>
      new LocalStoragePreferenceRepository(() => {
        throw new DOMException('blocked', 'SecurityError');
      }).getSelectedOwner(),
    PreferenceStorageUnavailableError
  );
  ...
  assert.throws(
    () => new LocalStoragePreferenceRepository(quota).setSelectedOwner('ZOSIA'),
    PreferenceStorageQuotaError
  );
});
